...
def get_mutual(
    ...
    itr = range(math.ceil(len(target_uids) / 100))  # type: ignore
    if progress:
        itr = progress(itr)
...
if __name__ == "__main__":
    ...
    friends_response = get_friends(user_id=786236223, fields=["nickname"])
    # print(get_friends(user_id=786236223))
    # print(get_mutual(source_uid=786236223, target_uid=202150635))
    active_users = [user["id"] for user in friends_response.items if not user.get("deactivated")]  # type: ignore
    print(len(active_users))
    mutual_friends = get_mutual(source_uid=238422411, target_uids=active_users, progress=tqdm)
